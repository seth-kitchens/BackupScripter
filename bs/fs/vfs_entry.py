...
class VFSEntryBS(nss.VFSEntry):
    ...
    def calc_included_folder_count(self):
        self.included_folders = sum(self.for_children(VFSEntryBS.calc_included_folder_count, recurse=False))
    ...
    def calc_included_file_count(self):
        self.included_files = sum(self.for_children(VFSEntryBS.calc_included_file_count, recurse=False))
        if self.is_included() and self.is_file():
    ...
    def calc_excluded_folder_count(self):
        self.excluded_folders = sum(self.for_children(VFSEntryBS.calc_excluded_folder_count, recurse=False))
    ...
    def calc_excluded_file_count(self):
        self.excluded_files = sum(self.for_children(VFSEntryBS.calc_excluded_file_count, recurse=False))
        if self.is_excluded() and self.is_file():
