...
class VFSExplorerView(psgu.GuiElement.iLayout, psgu.GuiElement):
    ...
    def __init__(self, object_id, vfs_explorer, read_only=False) -> None:
        ...
        self.vfs_explorer:fs.VFSExplorer = vfs_explorer
        self.vfs:fs.VFS = vfs_explorer.vfs
    ...
    def _get_row_details(self):
        ...
        row_details = [
            ...
            sg.Multiline(
                ...
                size=(psgu.g.explorer_list_width, 5),
                expand_x=True,
                disabled=True)
        ]
        return row_details
    ...
    def _get_row_listbox(self):
        ...
        item_list_size = (psgu.g.explorer_list_width, 12)
        sge_item_list = psgu.sg.Listbox(values=[],
            key=self.keys['Listbox'],
            enable_events=True,
            bind_return_key=True,
            size=item_list_size,
            expand_x=True,
            font=psgu.g.explorer_list_font,
    ...
    def define_events(self):
        ...
        def event_listbox(event_context:psgu.EventContext):
            ...
            item_list = event_context.values[self.keys['Listbox']]
            if len(item_list) <= 0:
                return
            if item_list[0][1] == '#':
                self.deselect()
            ...
            if same_clicked and is_double_click:
                event_context.event = self.key_rcm('ListboxFolder', 'Open')
        ...
        def event_open_folder(event_context:psgu.EventContext):
        ...
        def events_exit_folder(event_context:psgu.EventContext):
        ...
        def event_add_folder(event_context:psgu.EventContext):
            item_path = psgu.tk.browse_folder(event_context.window)
            if item_path == '':
                return
        ...
        def event_add_files(event_context:psgu.EventContext):
            item_paths = psgu.tk.browse_files(event_context.window)
            if len(item_paths) <= 0:
                return
        ...
        def events_exit_to_root(event_context:psgu.EventContext):
            self.vfs_explorer.exit_to_root()
            self.deselect()
            self.push(event_context.window)
        
        @self.eventmethod(self.keys['RemoveAll'])
        def event_remove_all(event_context:psgu.EventContext):
            if not psgu.popups.confirm(event_context.window_context, 'Remove All?'):
                return
            self.vfs.remove_all()
            self.vfs_explorer.refresh_current_dir()
            self.deselect()
    ...
    def refresh_display_list(self):
        ...
        for child in self.vfs_explorer.current_dir_children:
            ...
            e_size = psgu.utils.center_decimal_string(str(e_size_value), 2) + ' ' + e_size_symbol.rjust(2, ' ')
            display_table.add_row([
                str(i),
            ...
            i += 1
        display_table.trim_fields(psgu.g.explorer_list_width, [(2, 8)])
