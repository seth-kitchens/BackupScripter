from psgu.ge import InfoBuilder


class window_main:

    backup_scripter = InfoBuilder() \
        .header('Backup Scripter', 'Make custom single-file backup scripts') \
        .frame('Script File', [
            'Settings for the script file created']) \
        .frame('Backup File', [
            'Settings for the backup file created by the script.\n\n',
            'Archive Mode\n',
            '- Compile: Copy all included items to a directory first,'
            'then archives that directory.',
            'This takes up more memory and time, but results in a better compression ratio.\n',
            '- Append: Archive each included item directly into the archive. This does not copy',
            'the files before, so it takes up less space. May be faster. Likely results in a',
            'bigger backup file.']) \
        .frame('To Backup', [
    ...
    date_postfix = InfoBuilder() \
        ...
        .frame('Patterns', [
            'YYYY: Year\nMM: Month\nDD: Day\nHH: Hour(24)\nhh: Hour(12)\n',
            'mm: Minute\psgu: Second\nUU: Unix time'])
    
    backup_settings = InfoBuilder() \
        .header('Backup Settings') \
        .frame('Max Backups', [
            'Any files in the same backup destination folder with the same name,',
            'extension and date postfix (from a different time)',
            'will be identified as an existing backup.',
            'These files will count toward the max and may be deleted.']) \
        .frame('Recent Age', [
...
class window_manage_included:
    ...
    window = InfoBuilder() \
        .header('Manage Included') \
        .frame('Static Inclusion', [
            'Include or exclude files and folders here by manually selecting them.',
            'Excluding a root item',
        ...
        .frame('Matching Groups', [
            'Include files at script runtime based on patterns and conditions.',
            'These matching groups are named and applied in order.',
            'Use the arrows on the left to reorder them.',
            'You can view how the matching groups look at any point in the list with',
            'the "Preview Here" button (includes selected),',
            'or view all with the "Preview All" button.',
            'The "Resolve Here" and "Resolve All" buttons permanently transform the',
            'matching groups into static inclusion.',
            'In the details box, highlighted text shows selected options within',
            'the matching group that are taking effect.',
            'Orange highlighted text shows effects that are expected to be in error.'])

class matching_groups_list:

    window = InfoBuilder() \
        .header('Matching Groups') \
        .frame('Action', [
        ...
        .frame('Patterns', [
            ...
            '- Match Whole Name: Pattern must match whole name and',
            'not just be found within name\n',
        ...
        .frame('Apply If', [
            ...
            'not match the condition, the group\'s action (include/exclude) will not',
            'be applied to it']) \
