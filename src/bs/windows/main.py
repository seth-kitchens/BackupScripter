...
class WindowMain(sgu.AbstractBlockingWindow):
    ...
    def get_layout(self):
        frame_script_file = sgu.sg.FrameColumn('Script File', expand_x=True, layout=[
            self.row(sgu.ge.Filename('ScriptFilename', 'Filename').sg_kwargs_name(expand_x=True)),
            self.row(sgu.ge.Path('ScriptDestination', 'Destination').sg_kwargs_path(expand_x=True)),
            [
        ...
        frame_backup_file = sgu.sg.FrameColumn('Backup File', expand_x=True, layout=[
            self.row(sgu.ge.Filename('BackupFilename', 'Filename').sg_kwargs_name(expand_x=True)),
            [
                *self.row(sgu.ge.Input('BackupDatePostfix', 'Date Postfix').sg_kwargs_in(expand_x=True)),
                sgu.ge.Info(self.gem, info.date_postfix)
            ],
            self.row(sgu.ge.Path('BackupDestination', 'Destination').sg_kwargs_path(expand_x=True)),
            self.row(sgu.ge.Dropdown('ArchiveFormat', 'Archive Format', list(WindowMain.archive_exts.keys()))),
            self.row(sgu.ge.Radio('ArchiveMode', 'Archive Mode:', {'append': 'Append', 'compile': 'Compile'}))
        ])
        frame_backup_settings = sgu.sg.FrameColumn('Backup Settings', expand_y=True, layout=[
            [
                *self.row(sgu.ge.Input('MaxBackups', 'Max Backups', type='int', negative_invalid=True)),
                sg.Push(),
                sgu.ge.Info(self.gem, info.backup_settings, '?')
            ],
            self.row(sgu.ge.InputUnits('BackupRecentAge', 'Recent Age', sgu.units.Time, sgu.units.Time.DAY, store_as_degree=sgu.units.Time.SECOND, negative_invalid=True)),
            self.row(sgu.ge.InputUnits('BackupOldAge', 'Old Age', sgu.units.Time, sgu.units.Time.DAY, store_as_degree=sgu.units.Time.SECOND, negative_invalid=True)),
            [self.sge(sgu.ge.Checkbox('PullAgeFromPostfix', 'Pull Age From Filename'))],
            [sg.VPush()]
        ])
        ...
        column_included_numbers = sg.Column(element_justification='center', pad=0, layout=[
            self.row(sgu.ge.OutText('TotalFolders')),
            self.row(sgu.ge.OutText('TotalFiles'))
        ])
        column_included = sg.Column(element_justification='left', pad=0, layout=[
            ...
            [sg.Column(pad=0, expand_x=True, layout=[[sg.Text('Size'), sg.Push(), *self.row(sgu.ge.OutText('SizeIncluded'))]])]
        ])
        ...
        column_excluded_numbers = sg.Column(element_justification='center', pad=0, layout=[
            self.row(sgu.ge.OutText('TotalFoldersExcluded')),
            self.row(sgu.ge.OutText('TotalFilesExcluded'))
        ])
        column_excluded = sg.Column(pad=0, expand_y=True, layout=[
            ...
            [sg.Column(pad=0, expand_x=True, layout=[[sg.Text('Size'), sg.Push(), *self.row(sgu.ge.OutText('SizeExcluded'))]])]
        ])
        frame_ie = sgu.sg.FrameColumn('To Backup', layout=[
            [
                column_included,
                sg.VSeparator(),
                column_excluded
            ...
            [
                sg.Push(),
                *self.row(sgu.ge.Radio('IENumbers', text=None, options={'final':'Final', 'static':'Static', 'both':'Static/Final'}).load_value('final')),
                sg.Push()
            ],
        ...
        row_system = [
            sgu.ge.Info(self.gem, info.backup_scripter, 'Info', sg_kwargs={'size': system_button_size}),
    ...
    def define_menus(self):
        mb = self.menubar = sgu.sg.MenuBar()
        mb.unlock()
        pass
        mb.lock()
    ...
    def define_events(self):
        ...
        def event_radio_ie_numbers(context:sgu.WindowContext):
        ...
        def event_set_defaults(context:sgu.WindowContext):
        ...
        def event_load_defaults(context:sgu.WindowContext):
        ...
        def event_compression_type_chosen(context:sgu.WindowContext):
        ...
        def event_export_quit(context:sgu.WindowContext):
            self.pull(context.values)
            self.save(self.data)
        ...
        def event_export_script(context:sgu.WindowContext):
        ...
        def event_load_script(context:sgu.WindowContext):
            script_to_load = sgu.sg.browse_file(context.window)
        ...
        def event_compression_settings(context:sgu.WindowContext):
            pass

        @self.eventmethod('ManageIncluded')
        def event_manage_included(context:sgu.WindowContext):
            self.script_data.load_dict(self.data)
            window_manage_included = WindowManageIncluded(self.script_data, self.vfs_static)
            rv = sgu.WRC(window_manage_included.open(context))
            if rv.check_exit():
                return rv
    ...
    def create_script(self, context:sgu.WindowContext, auto_close=True):
        popup_title = 'Create Script'
        self.script_data.load_dict(self.data)
        popup_progress = sgu.ProgressWindow('Progress', header='Creating Script')
        ...
        try:
            ...
            success = False
            sgu.PopupBuilder().text(('Script Creation Failed!', 'Exception: ' + str(e))).title(popup_title).ok().open(context)
            raise e
        popup_progress.close(context)
        if success:
            popup = sgu.PopupBuilder().text('Script Created Successfully').title(popup_title).ok()
            if auto_close:
                popup.auto_ok()
            popup.open(context)
        return success
    ...
    def refresh_ie(self, window):
        ...
        isz_static = sgu.units.Bytes(vfsdata_static.included_size, sgu.units.Bytes.B).get_best(1, 0.1)
        ef_static = vfsdata_static.excluded_file_count
        eF_static = vfsdata_static.excluded_folder_count
        esz_static = sgu.units.Bytes(vfsdata_static.excluded_size, sgu.units.Bytes.B).get_best(1, 0.1)

        if_final = vfsdata_final.included_file_count
        iF_final = vfsdata_final.included_folder_count
        isz_final = sgu.units.Bytes(vfsdata_final.included_size, sgu.units.Bytes.B).get_best(1, 0.1)
        ef_final = vfsdata_final.excluded_file_count
        eF_final = vfsdata_final.excluded_folder_count
        esz_final = sgu.units.Bytes(vfsdata_final.excluded_size, sgu.units.Bytes.B).get_best(1, 0.1)
